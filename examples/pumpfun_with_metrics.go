...
//
// Run: GRPC_URL=host:443 GRPC_TOKEN=your_token go run examples/pumpfun_with_metrics.go
...
func main() {
	...
	go reportStats("Performance Stats (10s window)", &eventCount, &totalLatency, &minLatency, &maxLatency, &lastCount)
	...
	for {
		...
		case ev, ok := <-sub.Events:
			...
			fmt.Printf("================================================\n")
			fmt.Printf("Event: %s\n", ev.Type)
			if trade := ev.AsPumpFunTrade(); trade != nil {
				fmt.Printf("  mint  : %s\n", trade.Mint)
				fmt.Printf("  sol_amount: %d\n", trade.SolAmount)
				fmt.Printf("  token_amount: %d\n", trade.TokenAmount)
				fmt.Printf("  user  : %s\n", trade.User)
			}
			if create := ev.AsPumpFunCreate(); create != nil {
				fmt.Printf("  name  : %s\n", create.Name)
				fmt.Printf("  symbol: %s\n", create.Symbol)
				fmt.Printf("  mint  : %s\n", create.Mint)
			}
